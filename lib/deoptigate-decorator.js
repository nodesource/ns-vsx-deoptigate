...
const HIGH_SEVERITY_THRESHOLD = 2
  ...
  _calculateDeoptMarkers(info) {
    ...
    if (this._config.showDeoptimizations) {
      for (const deopt of fileInfo.deopts.values()) {
        const severity = deopt.severity
        if (highSeverityOnly && severity < HIGH_SEVERITY_THRESHOLD) continue
        ...
        markers.push({ start: offset, end: offset + 1, details, severity, type: 'deopt' })
      }
    }

    if (this._config.showInlineCaches) {
      for (const ic of fileInfo.ics.values()) {
        const severity = ic.severity
        if (highSeverityOnly && severity < HIGH_SEVERITY_THRESHOLD) continue
        const details = icHoverMessage(ic)
        const pos = new Position(ic.line - 1, ic.column - 1)
        const offset = this._activeEditor.document.offsetAt(pos)
        markers.push({ start: offset, end: offset + 1, details, severity, type: 'ic' })
      }
    }
